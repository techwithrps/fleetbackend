...
class EquipmentModel {
  ...
  static isBinarySqlType(dataType) {
    const t = String(dataType || "").toLowerCase();
    return t.includes("binary") || t === "image" || t === "varbinary";
  }

  static async getFleetDocumentColumnTypes() {
    ...
    if (schema === "fleet") {
      ...
      return result.recordset.map((row) => {
        ...
        return {
          ...
          IMAGE: mapDoc(row.IMAGE, "IMAGE"),
          FITNESS_DOC: mapDoc(row.FITNESS_DOC, "FITNESS_DOC"),
          RC_DOC: mapDoc(row.RC_DOC, "RC_DOC"),
          INSURANCE_DOC: mapDoc(row.INSURANCE_DOC, "INSURANCE_DOC"),
          PERMIT_A: mapDoc(row.PERMIT_A, "PERMIT_A"),
          PERMIT_B: mapDoc(row.PERMIT_B, "PERMIT_B")
        };
      });
    }
  ...
  static async create(data) {
    ...
    if (schema === "fleet") {
      const docColumnTypes = await this.getFleetDocumentColumnTypes();
      for (const field of this.DOCUMENT_COLUMNS) {
        if (Buffer.isBuffer(data[field]) && !this.isBinarySqlType(docColumnTypes[field])) {
          throw new Error(
            `Document column ${field} is not VARBINARY(MAX). Run alter_tables_for_binary_storage.sql on this database.`
          );
        }
      }
      ...
      request
        ...
        .input("image", sql.VarBinary(sql.MAX), Buffer.isBuffer(data.IMAGE) ? data.IMAGE : null)
        ...
        .input("fitness_doc", sql.VarBinary(sql.MAX), Buffer.isBuffer(data.FITNESS_DOC) ? data.FITNESS_DOC : null)
        .input("validity", sql.Date, data.VALIDITY ? new Date(data.VALIDITY) : null)
        .input("rc_doc", sql.VarBinary(sql.MAX), Buffer.isBuffer(data.RC_DOC) ? data.RC_DOC : null)
        .input("insurance_doc", sql.VarBinary(sql.MAX), Buffer.isBuffer(data.INSURANCE_DOC) ? data.INSURANCE_DOC : null)
        .input("permit_a", sql.VarBinary(sql.MAX), Buffer.isBuffer(data.PERMIT_A) ? data.PERMIT_A : null)
        .input("permit_b", sql.VarBinary(sql.MAX), Buffer.isBuffer(data.PERMIT_B) ? data.PERMIT_B : null)
  ...
  static async update(equipmentId, data) {
    ...
    if (schema === "fleet") {
      ...
      for (const field of this.DOCUMENT_COLUMNS) {
        ...
        if (!this.isBinarySqlType(docColumnTypes[field])) {
          throw new Error(
            `Document column ${field} is not VARBINARY(MAX). Run alter_tables_for_binary_storage.sql on this database.`
          );
        }

        const param = field.toLowerCase();
        request.input(param, sql.VarBinary(sql.MAX), value);
        docSetClauses.push(`${field} = @${param}`);
      }

      const docSetSql = docSetClauses.length ? `,\n            ${docSetClauses.join(",\n            ")}` : "";
  ...
  static async getDocumentRaw(equipmentId, docType) {
    const validDocs = ["IMAGE", "FITNESS_DOC", "RC_DOC", "INSURANCE_DOC", "PERMIT_A", "PERMIT_B"];
    if (!validDocs.includes(docType)) throw new Error("Invalid document type");

    const schema = await this.detectSchema();
    if (schema === "fleet") {
      const result = await pool.request()
        .input("equipment_id", sql.Numeric(18, 0), equipmentId)
        .query(`SELECT ${docType} FROM FLEET_EQUIPMENT_MASTER WHERE EQUIPMENT_ID = @equipment_id`);
      return result.recordset[0]?.[docType] || null;
    }
