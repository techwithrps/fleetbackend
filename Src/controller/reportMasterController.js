...
const { applyLocationFilter, applyLocationFilterByTable } = require("../utils/queryHelper");
...
class ReportMasterController {
  static async getDashboardSummary(req, res) {
    try {
      ...
      const reqVehicles = pool.request();
      const filterVehicle = await applyLocationFilterByTable(reqVehicles, user, {
        tableName: "FLEET_EQUIPMENT_MASTER",
        alias: "v",
      });

      const reqJobOrders = pool.request();
      const filterJob = await applyLocationFilterByTable(reqJobOrders, user, {
        tableName: "JOB_ORDER",
        alias: "jo",
      });

      const reqExpenses = pool.request();
      const filterExpenseVehicles = await applyLocationFilterByTable(reqExpenses, user, {
        tableName: "FLEET_EQUIPMENT_MASTER",
        alias: "v",
      });

      const reqTrips = pool.request();
      const filterTrip = applyLocationFilter(reqTrips, user, "tr");

      const reqInventory = pool.request();
      const filterInventory = await applyLocationFilterByTable(reqInventory, user, {
        tableName: "ITEM_MASTER",
        alias: "im",
      });

      const [vehiclesRes, maintenanceRes, costRes, tripsRes, inventoryRes] = await Promise.all([
        reqVehicles.query(`SELECT COUNT(1) AS total FROM dbo.FLEET_EQUIPMENT_MASTER v WHERE 1=1 ${filterVehicle}`),
        reqJobOrders.query(`SELECT COUNT(DISTINCT VEHICLE_NO) AS total FROM dbo.JOB_ORDER jo WHERE jo.STATUS = 'OPEN' AND jo.VEHICLE_NO IS NOT NULL AND jo.VEHICLE_NO <> '' ${filterJob}`),
        reqExpenses.query(`
          SELECT SUM(ebd.TOTAL_AMOUNT) AS total
          FROM dbo.EXPENSE_BOOKING_DETAILS ebd
          INNER JOIN dbo.EXPENSE_BOOKING eb ON ebd.EXPENSE_ID = eb.EXPENSE_ID
          WHERE eb.EQUIPMENT_NO IN (
            SELECT v.EQUIPMENT_NO FROM dbo.FLEET_EQUIPMENT_MASTER v WHERE 1=1 ${filterExpenseVehicles}
          )
        `),
        reqTrips.query(`SELECT COUNT(1) AS total FROM dbo.transport_requests tr WHERE 1=1 ${filterTrip}`),
        reqInventory.query(`SELECT SUM(ISNULL(im.QNTY, 0) * ISNULL(im.UNIT_PRICE, 0)) AS total FROM dbo.ITEM_MASTER im WHERE 1=1 ${filterInventory}`)
      ]);

      res.json({
        success: true,
        data: {
          totalVehicles: vehiclesRes.recordset[0]?.total || 0,
          underMaintenance: maintenanceRes.recordset[0]?.total || 0,
          totalCost: costRes.recordset[0]?.total || 0,
          totalTrips: tripsRes.recordset[0]?.total || 0,
          totalInventoryValue: inventoryRes.recordset[0]?.total || 0
        }
      });
    } catch (error) {
      console.error("Dashboard summary stats error:", error);
