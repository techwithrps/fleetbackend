...
// ─── Cache soft-delete check (one-time at startup) ───────────────────────────
let _softDeleteCached = null;
const hasUserSoftDelete = async () => {
  if (_softDeleteCached !== null) return _softDeleteCached;
  ...
  `);
  _softDeleteCached = !!result.recordset[0]?.has_soft_delete;
  return _softDeleteCached;
};

// ─── Fetch locations for a user ───────────────────────────────────────────────
...
// ─── Get user access context (parallelized for admin) ────────────────────────
const getUserAccessContext = async (userId, role) => {
  try {
    if (role && role.toLowerCase() === "admin") {
      // Run both admin queries in parallel
      const [locResult, pageResult] = await Promise.all([
        pool.request().query(
          "SELECT CAST(LOCATION_ID AS NUMERIC(18,0)) as id FROM dbo.LOCATION_MASTER WHERE LOCATION_ID IS NOT NULL"
        ),
        pool.request().query(
          "SELECT PAGE_ID, PAGE_NAME FROM dbo.PAGE_MASTER"
        ),
      ]);
    ...
    // Run user meta + permissions in parallel
    const [userMeta, permResult] = await Promise.all([
      pool
        .request()
        .input("user_id", sql.Int, userId)
    ...
    const viewedPages = permResult.recordset.filter((r) => r.can_view);
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    // Run password check and access context fetch in parallel
    const [isPasswordValid, accessContext] = await Promise.all([
  ...
  try {
    ...
    if (user.role?.toLowerCase() === "admin") {
      const locResult = await pool.request().query(
        "SELECT CAST(LOCATION_ID AS NUMERIC(18,0)) as id, LOCATION_NAME as name FROM dbo.LOCATION_MASTER WHERE LOCATION_ID IS NOT NULL"
      );
      return res.json({
        success: true,
        isAdmin: true,
        locations: [{ id: "ALL", name: "ALL LOCATIONS" }, ...locResult.recordset],
      });
    }
...
exports.switchLocation = async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, token: newToken, terminalId: locationId || null });
  } catch (error) {
